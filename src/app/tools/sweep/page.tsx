...
function StreetSweeperTruck({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 260 130" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      {/* Main truck body */}
...
export default function Sweep() {
  const [sweepState] = useState<SweepState | null>(() => getSweepState(new Date()));
        ...
        {pageState !== 'revealed' && (
          ...
          >
            ...
            {pageState === 'sweeping' && config && (
              <div className="sweep-trail" style={{ '--trail-color': config.trailColor } as React.CSSProperties} />
            )}
          </div>
        )}
